...
const LogsServicesSchema = LogsServicesCreateBody

const logsServices = (): ToolBase<typeof LogsServicesSchema, Schemas._LogsServicesResponse> => ({
    name: 'logs-services',
    schema: LogsServicesSchema,
    handler: async (context: Context, params: z.infer<typeof LogsServicesSchema>) => {
        const projectId = await context.stateManager.getProjectId()
        const body: Record<string, unknown> = {}
        if (params.query !== undefined) {
            body['query'] = params.query
