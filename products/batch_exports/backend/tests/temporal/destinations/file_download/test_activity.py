...
TEST_REGION = os.getenv("AWS_DEFAULT_REGION", "us-east-1")
TEST_ROLE = os.getenv("BATCH_EXPORTS_FILE_DOWNLOAD_ROLE", "test-role")
...
async def test_export_to_file_download_bucket_puts_data_into_s3(
    ...
    """Test that export_to_file_download_bucket_with_temporay_credentials exports data to S3.
...
async def test_generate_file_downloads_creates_file_download_records(
    ...
    for key in test_keys:
        await s3_client.put_object(Bucket=bucket_name, Key=key, Body=b"test-data")

    try:
        inputs = GenerateFileDownloadsInputs(
            team_id=ateam.pk,
            batch_export_id=batch_export_id,
            s3_bucket=S3Bucket(name=bucket_name, region=TEST_REGION),
            aws_role=TEST_ROLE,
        ...
        for fd in file_downloads:
            ...
            assert str(fd.id) in file_download_ids
