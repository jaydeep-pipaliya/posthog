...
| `posthog:logs-alerts-attach-destination`                              | Wire the alert to Slack or webhook.                                           | Step 5 — ship.     |
...
2. Call `posthog:logs-alerts-attach-destination` to wire it to a notification target. **An alert with no destination
