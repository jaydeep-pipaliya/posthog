...
Requires `TeamSecretTokenAuthentication` (legacy `Team.secret_api_token` field). Rate limited at 600/minute (overridable per team via `LOCAL_EVAL_RATE_LIMITS`). Checks billing quotas via `list_limited_team_attributes`.
