...
- **Cross-tenant id in custom @action** (`test_cross_tenant_id_in_action`) — attacker invokes a viewset's custom `@action` endpoint (e.g., `copy_between_projects`, `publish_draft`, `simulate`) with a `<thing>_id` value pointing at a victim's resource. Catches IDORs on action endpoints with **non-ModelSerializer** request bodies (e.g., `serializers.Serializer` subclasses used purely as request shapes). Discovery walks `viewset_cls.get_extra_actions()` and pulls the request serializer from `@extend_schema(request=...)` via drf-spectacular's closure introspection. Detection adds a "name-pattern" path that resolves `<thing>` (with `source_/target_/from_/to_/new_/old_/parent_/child_` prefix-stripping) to one or more tenant-scoped model names by case-insensitive suffix match against the semgrep allowlist. Ambiguous matches (e.g., `template` → DashboardTemplate / MessageTemplate / HogFlowTemplate) emit one record per candidate so the runtime fans out. The runtime check is softer than for vanilla CRUD because action semantics vary: 4xx → pass, 5xx → skip-loud, 2xx → assert no victim sentinel leaks in the response body. Skip a viewset+action via `IDOR_ACTION_SKIP_LIST` (keys formatted `ViewSetName.action_method_name`) with categories `ACTION_NOT_TESTABLE`, `INTENTIONAL_CROSS_TENANT`, `BODY_SYNTHESIS_INFEASIBLE`, `REQUIRES_FILESYSTEM_OR_TEMPORAL`.
