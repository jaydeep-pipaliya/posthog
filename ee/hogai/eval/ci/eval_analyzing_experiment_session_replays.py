...
@pytest.fixture
def experiment_with_session_replays(
    demo_org_team_user,
) -> Callable[[dict[str, Any]], Awaitable[tuple[Experiment, dict[str, Any]]]]:
    """Create experiment with feature flag matching mock template structure."""
    _, team, user = demo_org_team_user

    async def setup(mock_template: dict[str, Any]) -> tuple[Experiment, dict[str, Any]]:
        ...
        return experiment, mock_template
...
def _extract_session_replay_analysis(state: AssistantState, mock_template: dict[str, Any] | None) -> dict[str, Any]:
    ...
    for msg in state.messages:
        ...
        if isinstance(msg, AssistantToolCallMessage) and msg.content:
            ...
            try:
                # Tool content might be plain text or JSON
                content = msg.content

                # Check for mock recordings structure
                if mock_template and mock_template.get("recordings"):
                    # Extract variant from the message context
                    # The mock returns recordings directly as a list
                    if isinstance(content, str) and "recording" in content.lower():
                        ...
                        try:
                            ...
                            if isinstance(parsed, list):
                                # Determine which variant based on order of calls
                                variants = mock_template.get("variants", [])
                                existing_variants = list(result["recordings"].keys())
                                if len(existing_variants) < len(variants):
                                    variant_key = variants[len(existing_variants)]
                                    result["recordings"][variant_key] = parsed
                        except (json_module.JSONDecodeError, ValueError):
                            # Not JSON, might be formatted text
                            pass

                # Real tool returns formatted text with recording metadata
                # We can store the raw content as "recordings" for the scorer
                if "User:" in content or "Duration:" in content or "Activity:" in content:
                    # This looks like formatted recording metadata
                    # Try to determine variant from recent filter calls
                    recent_filters = list(result["filters"].keys())
                    if recent_filters and recent_filters[-1] not in result["recordings"]:
                        # Associate this result with the most recent filter variant
                        result["recordings"][recent_filters[-1]] = content
...
def call_analyzing_session_replays_skill(
    demo_org_team_user, experiment_with_session_replays
    ...
    async def execute(input: dict[str, Any]) -> dict[str, Any]:
        ...
        if mock_template:
            experiment, _ = await experiment_with_session_replays(mock_template)
        ...
        # Run graph - the agent will use real filter_session_recordings tool
        # No mocking needed as the eval tests agent behavior, not tool implementation
        state = await graph.ainvoke(
            AssistantState(messages=[HumanMessage(content=user_input)]),
            config=RunnableConfig(configurable={"thread_id": conversation.id}, recursion_limit=24),
        )
...
class FilterConstructionCorrectness(Scorer):
    ...
    def _evaluate(self, output: dict | None, expected: dict | None) -> Score:
        ...
        for variant in variants:
            ...
            events = variant_filter.get("events")
            if not isinstance(events, list) or not events:
                issues.append(f"Variant '{variant}' missing events array")
                variant_scores.append(0.0)
                continue
            ...
            for event in events:
                if not isinstance(event, dict):
                    continue
                if event.get("id") == "$feature_flag_called":
            ...
            # Check date_from exists
            if "date_from" not in variant_filter:
                variant_errors.append(f"Variant '{variant}' missing date_from field")
