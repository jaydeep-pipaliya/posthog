...
# Register with hogli's CLI group if the framework is importable. In a minimal
# CI venv (no posthog deps), `hogli.cli` may not be present — skipping silently
# is correct: the standalone entry script invokes ``cmd_lint_workflows`` directly.
try:
    from hogli.cli import cli as _hogli_cli  # type: ignore[import-not-found]

    _hogli_cli.add_command(cmd_lint_workflows)
except ImportError:
    pass
