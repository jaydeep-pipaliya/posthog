...
class TestValidateFacadeAlternation:
    ...
    def test_no_canonical_block(self, tmp_path: Path) -> None:
        # A tach with only a legacy-leak block — no canonical alternation
        # exists yet, so this validator stays quiet (TachCheck handles the
        # per-product side).
...
"""
    ...
    def test_stale_entry_product_missing(self, tmp_path: Path) -> None:
        _mkproduct(tmp_path, "alpha", isolated=True)
        # beta is in the alternation but does not exist on disk
    ...
    def test_isolated_product_missing_from_alternation(self, tmp_path: Path) -> None:
        _mkproduct(tmp_path, "alpha", isolated=True)
        _mkproduct(tmp_path, "beta", isolated=True)
        _mkproduct(tmp_path, "gamma", isolated=True)  # isolated but not in alternation
        issues = validate_facade_alternation(_CANONICAL_BLOCK, tmp_path)
    ...
    def test_legacy_block_alone_is_not_canonical(self, tmp_path: Path) -> None:
        # An isolated product with only a legacy-leak block (non-canonical
        # expose) must still be flagged as missing from the canonical
        # alternation.
        _mkproduct(tmp_path, "alpha", isolated=True)
        _mkproduct(tmp_path, "beta", isolated=True)
...
"""
    ...
    def test_two_backslash_form_parses(self, tmp_path: Path) -> None:
        # Real tach.toml on disk uses two literal backslashes (`\\.`).
        # The Python-string fixture above only has one when triple-quoted,
        # so simulate the on-disk form here.
        _mkproduct(tmp_path, "alpha", isolated=True)
        on_disk = (
            "[[interfaces]]\n"
            'expose = [\n    "backend\\\\.facade.*",\n    "backend\\\\.presentation\\\\.views.*",\n]\n'
    ...
    def test_lenient_products_ignored(self, tmp_path: Path) -> None:
        # Non-isolated products without contracts.py should never trigger
        # the "missing from alternation" rule.
        _mkproduct(tmp_path, "alpha", isolated=True)
        _mkproduct(tmp_path, "beta", isolated=True)
...
class TestNamesFromPattern:
    @pytest.mark.parametrize(
        ...
        [
            ...
            # Unsupported / unrelated forms — return empty set
            ("posthog.api", set()),
